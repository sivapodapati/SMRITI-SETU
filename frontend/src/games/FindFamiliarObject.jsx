...
// Similar items pool for Game 3 high difficulty distractors (e.g. colored bowls/cups)
const SIMILAR_POOL = [
  { id: "tea_red", emoji: "🥛", names: { English: "Glass Cup", Assamese: "গিলাচ", Bengali: "গ্লাস", Meitei: "ꯒ꯭ꯂꯥꯁ", Nepali: "गिलास", Mizo: "No", Khasi: "Gilas", Garo: "Gilas", Hindi: "कांच का गिलास" } },
  { id: "tea_yellow", emoji: "☕", names: { English: "Coffee Mug", Assamese: "কফি মগ", Bengali: "কফি মগ", Meitei: "ꯀꯣꯐꯤ ꯃꯒ", Nepali: "कॉफी मग", Mizo: "Mug", Khasi: "Mug kofi", Garo: "Mug", Hindi: "कॉफ़ी मग" } },
  { id: "rice_plate", emoji: "🍽️", names: { English: "Plate", Assamese: "কাঁহী", Bengali: "থালা", Meitei: "ꯄ꯭ꯂꯦꯠ", Nepali: "प्लेट", Mizo: "Thleng", Khasi: "Pliang", Garo: "Thala", Hindi: "थाली" } }
];

export default function FindFamiliarObject({ patientId, patientInfo, levelConfig, supportLevel, locData, onExit }) {
  ...
  // Construct regional objects pool dynamically
  const OBJECTS_POOL = getRegionalAssets(state, lang).map(item => ({
    id: item.id,
    emoji: item.emoji,
    names: {
      [lang]: item.name,
  ...
  const getLocalizedPrompt = (obj) => {
    if (!obj) return "";
    const objName = obj.names[lang] || obj.names["English"];
    switch (lang) {
      case "Assamese":
        return `${objName}টো বাছনি কৰক`;
      case "Bengali":
        return `${objName}টি নির্বাচন করুন`;
      case "Meitei":
        return `${objName} ꯈꯟꯕꯤꯌꯨ`;
      case "Nepali":
        return `${objName} छान्नुहोस्`;
      case "Mizo":
        return `${objName} thlang rawh`;
      case "Khasi":
        return `Jied ïa ka ${objName}`;
      case "Garo":
        return `${objName}ko bisarobo`;
      default:
        return `Find the ${objName}`;
    }
  };

  const initializeGame = () => {
    // Select a target object
    const targetObj = OBJECTS_POOL[Math.floor(Math.random() * OBJECTS_POOL.length)];
    setTarget(targetObj);

    // Pick distractors
    let pool = OBJECTS_POOL.filter((o) => o.id !== targetObj.id);
    if (config.use_similar_distractors) {
      pool = [...pool, ...SIMILAR_POOL];
    }
    
    const shuffled = pool.sort(() => Math.random() - 0.5);
    const dist = shuffled.slice(0, config.choices_count - 1);
    
    // Combine target + distractors and shuffle
    const gameChoices = [targetObj, ...dist].sort(() => Math.random() - 0.5);
    ...
    setMessage(getLocalizedPrompt(targetObj));
  };
  ...
  const handleChoiceClick = (choiceId) => {
    ...
    if (choiceId === target.id) {
      ...
      setMessage(`${getTranslation(lang, "wrong_match")} ${getLocalizedPrompt(target)}`);
    }
  };
  ...
  const useHint = () => {
    ...
    setMessage(`${getTranslation(lang, "next_item_hint")}: ${target.emoji}`);
    setTimeout(() => {
      setHintActive(false);
      setMessage(getLocalizedPrompt(target));
    }, 2000);
  };
  ...
  const finishGame = async (success) => {
    ...
    const engagementScore = Math.min(100, Math.max(0, 100 - moves * 3 - hintsUsed * 5));
    ...
    <div className="min-h-screen bg-[#FDFBF7] flex items-center justify-center p-4 sm:p-6 select-none">
      <div className="w-full max-w-3xl bg-white rounded-[2rem] border border-[#EBE7DF] shadow-sm p-6 md:p-10 flex flex-col">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900">{locData?.title || "Find the Familiar Object"}</h1>
        ...
        {gameState === "intro" ? (
          <div className="text-center">
            ...
            <div className="bg-teal-50/50 border border-teal-150 rounded-2xl p-6 mb-6 text-left">
              ...
              <p className="text-sm font-semibold text-slate-500">Find the target item among {config.choices_count} choices.</p>
            </div>
          ...
          <div>
            ...
            {/* Localized Prompt */}
            <div className="bg-[#FAF8F5] border border-[#EBE7DF] rounded-2xl p-5 mb-6 text-center text-2xl font-extrabold text-slate-800 flex flex-col md:flex-row items-center justify-center gap-4">
              <span>{message}</span>
              {target && (
                <VoiceButton text={getLocalizedPrompt(target)} language={locData.language} className="py-2.5 px-5 text-sm !bg-teal-50 hover:!bg-teal-150 !text-teal-700 shadow-none border border-teal-100" />
              )}
            </div>
            ...
            <div className="grid grid-cols-3 gap-6 mb-8 justify-center">
              {choices.map((choice) => {
                const isTarget = target && choice.id === target.id;
                const highlight = hintActive && isTarget;
                return (
                  <button
                    key={choice.id}
                    ...
                    className={`aspect-square rounded-2xl border-2 flex items-center justify-center transition-all bg-[#FAF8F5] hover:bg-[#F3EFE9] border-[#EBE7DF] active:scale-95 text-6xl md:text-7xl cursor-pointer ${
                      highlight ? "border-emerald-500 bg-emerald-50 scale-105" : ""
                    }`}
                  >
                    {choice.emoji}
                  </button>
